...
line.Draw();}
RGraph.DrawInGraphLabels(this);RGraph.DrawCrosshairs(this);RGraph.Annotate(this);RGraph.ShowZoomWindow(this);}
RGraph.Bar.prototype.DrawAxes = function ()
{
var gutter = this.gutter;this.context.beginPath();this.context.strokeStyle = this.Get('chart.axis.color');this.context.lineWidth = 1;if(this.Get('chart.yaxispos') == 'right'){
...
{
this.context.lineWidth = this.Get('chart.linewidth');this.context.strokeStyle = this.Get('chart.strokecolor');this.context.fillStyle = this.Get('chart.colors')[0];var prevX = 0;var prevY = 0;var gutter = this.gutter;if(this.Get('chart.ymax')){
...
for (i=0; i<this.data.length; ++i){
var width = (this.canvas.width - (2 * gutter) ) / this.data.length;;var height = (RGraph.array_sum(this.data[i]) / this.max) * (this.canvas.height - (2 * gutter) );var orig_height = height;if(this.Get('chart.xaxispos') == 'center'){
height /= 2;}
var x = (i * width) + gutter;var y = this.Get('chart.xaxispos') == 'center' ? (this.canvas.height / 2) - height : this.canvas.height - height - gutter;var hmargin = this.Get('chart.hmargin');var gutter = gutter;if(height < 0){
...
var barWidth = width - (2 * hmargin);this.context.strokeStyle = this.Get('chart.strokecolor');this.context.fillStyle = this.Get('chart.colors')[0];if(variant == 'sketch'){
this.context.lineCap = 'round';var sketchOffset = 3;this.context.beginPath();this.context.strokeStyle = this.Get('chart.colors')[0];this.context.moveTo(x + hmargin + 2, y + height - 2);this.context.lineTo(x + hmargin , y - 2);this.context.moveTo(x + hmargin - 3, y + -2 + (this.data[i] < 0 ? height : 0));this.context.bezierCurveTo(x + ((hmargin + width) * 0.33),y + 5 + (this.data[i] < 0 ? height - 10: 0),x + ((hmargin + width) * 0.66),y + 5 + (this.data[i] < 0 ? height - 10 : 0),x + hmargin + width + -1, y + 0 + (this.data[i] < 0 ? height : 0));this.context.moveTo(x + hmargin + width - 2, y + -2);this.context.lineTo(x + hmargin + width - 3, y + height - 3);for (var r=0.2; r<=0.8; r+=0.2){
...
this.coords[i] = [x, y, width, height];if(document.all && this.Get('chart.shadow')){
...
this.context.strokeStyle = this.Get('chart.strokecolor');this.context.fillStyle = this.Get('chart.colors')[j];var height = (this.data[i][j] / this.max) * (this.canvas.height - (2 * this.gutter) );if(this.Get('chart.xaxispos') == 'center'){
height /= 2;}
var totalHeight = (RGraph.array_sum(this.data[i]) / this.max) * (this.canvas.height - this.Get('chart.hmargin') - (2 * this.gutter));this.coords.push([x, y, width, height]);if(document.all && this.Get('chart.shadow')){
...
this.context.strokeStyle = this.Get('chart.strokecolor');this.context.fillStyle = this.Get('chart.colors')[j];var individualBarWidth = (width - (2 * hmargin)) / this.data[i].length;var height = (this.data[i][j] / this.max) * (this.canvas.height - (2 * this.gutter) );if(this.Get('chart.xaxispos') == 'center'){
height /= 2;}
var startX = x + hmargin + (j * individualBarWidth);var startY = (this.Get('chart.xaxispos') == 'bottom' ? this.canvas.height : (this.canvas.height / 2) + this.gutter) - this.gutter - height;if(height < 0){
...
{
var e = RGraph.FixEventObject(document.all ? event : e);if(e.button != 0) return;e = RGraph.FixEventObject(document.all ? event : e);var canvas = document.getElementById(this.id);var obj = canvas.__object__;RGraph.Redraw();var mouseCoords = RGraph.getMouseXY(e);for (var i=0; i<obj.coords.length; i++){
var mouseX = mouseCoords[0];var mouseY = mouseCoords[1];var left = obj.coords[i][0];var top = obj.coords[i][1];var width = obj.coords[i][2];var height = obj.coords[i][3];if(mouseX >= (left + 5  ) && mouseX <= (left + width - 5) && mouseY >= top && mouseY <= (top + height) ){
