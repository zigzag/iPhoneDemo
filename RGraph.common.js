...
RGraph.Text(context, null, '5', centerx, vpos, text, vCenter, 'center', null, null, null, true);
...
{
e = RGraph.FixEventObject(document.all ? event : e);if(e.ctrlKey) return true;RGraph.Contextmenu(obj.canvas, obj.Get('chart.contextmenu'), e);return false;}
} else {
...
{
if(document.all){
e.pageX = (event.clientX + document.body.scrollLeft);e.pageY = (event.clientY + document.body.scrollTop);e.target = event.srcElement;if(!document.body.scrollTop && document.documentElement.scrollTop){
...
{
var e = RGraph.FixEventObject(document.all ? event : e);var obj = e.target.__object__;var coords = RGraph.getMouseXY(e);var x = coords[0];var y = coords[1];var gutter = obj.Get('chart.gutter');var width = canvas.width;var height = canvas.height;obj.context.lineWidth = 1;if(
...
var canvas = e.target.__canvas__;var context = canvas.getContext('2d');var obj = canvas.__object__;var tmp = canvas;var coords = RGraph.getCanvasXY(canvas);var factor = obj.Get('chart.zoom.factor') - 1;var x = coords[0];var y = coords[1];var img = document.createElement('img');img.style.border = '2px dashed gray';img.style.width = canvas.width + 'px';img.style.height = canvas.height + 'px';img.style.position = 'absolute';img.style.left = x + 'px';img.style.top = y + 'px';img.style.backgroundColor = 'white';img.style.opacity = obj.Get('chart.zoom.fade.in') ? 0 : 1;img.src = canvas.toDataURL();document.body.appendChild(img);RGraph.Registry.Set('chart.zoomedimage', img);img.onclick = function (e)
{
...
for (var i=1; i<=frames; ++i){
setTimeout("RGraph.Registry.Get('chart.zoomedimage').style.boxShadow = 'rgba(0,0,0," + Number(i / frames) / 2 + ") 3px 3px 3px'", i * delay);setTimeout("RGraph.Registry.Get('chart.zoomedimage').style.MozBoxShadow = 'rgba(0,0,0," + Number(i / frames) / 2 + ") 3px 3px 3px'", i * delay);setTimeout("RGraph.Registry.Get('chart.zoomedimage').style.WebkitBoxShadow = 'rgba(0,0,0," + Number(i / frames) / 2 + ") 3px 3px 3px'", i * delay);}
}
}
...
canvas.onmousemove = function (e)
{
e = RGraph.FixEventObject(document.all ? event : e);var obj = e.target.__object__;var canvas = obj.canvas;var context = obj.context;var coords = RGraph.getMouseXY(e);if(!RGraph.Registry.Get('chart.zoomed.div')){
...
{
RGraph.FixEventObject(document.all ? event : e);var obj = e.target.__object__;var canvas = obj.canvas;var context = obj.context;var mouseCoords = RGraph.getMouseXY(e);var x = mouseCoords[0] - obj.centerx;var y = mouseCoords[1] - obj.centery;var r = obj.radius;var theta = Math.atan(y / x);var hyp = y / Math.sin(theta);var angles = obj.angles;var ret = [];theta *= 57.3
